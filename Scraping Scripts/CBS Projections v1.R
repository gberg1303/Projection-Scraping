...
### Get the Tables
qb_cbs <- getURL(paste0("https://www.cbssports.com/fantasy/football/stats/QB/", season, "/season/projections/ppr/"))
qb_cbs <- readHTMLTable(qb_cbs, stringsAsFactors = FALSE)$`NULL`
 #qb_cbs[2:16] <- as.numeric(unlist(qb_cbs[2:16]))
rb_cbs <- getURL(paste0("https://www.cbssports.com/fantasy/football/stats/RB/",season,"/season/projections/ppr/"))
